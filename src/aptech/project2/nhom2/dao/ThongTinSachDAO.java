...
public class ThongTinSachDAO {
    ...
    // public List<ThongTinSach> findByTen(String ten) { //thay thế bởi Stream trên
    // GUI
    // List<ThongTinSach> list = new ArrayList<>();
    ...
    public boolean update(ThongTinSach sach) {
        ...
        try {
            if (sach.getAnh() == null) {
                stm = con.prepareStatement(
                        "update thong_tin_sach set ten = ?, id_tac_gia = ?, id_danh_muc = ?, so_luong = ?, id_nha_xuat_ban = ?, mo_ta = ? where id = ?");
                stm.setString(1, sach.getTen());
                stm.setInt(2, sach.getTacGia().getId());
                stm.setInt(3, sach.getDanhMucSach().getId());
                stm.setInt(4, sach.getSoLuong());
    ...
    public void updateMoTa(String moTa, int id) {
        ...
        try {
            stm = con.prepareStatement("update thong_tin_sach set mo_ta = ? where id = ?");
